...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.users.store') }}" class="saas-card max-w-2xl space-y-5">
        @csrf
        @include('admin.users._form', ['user' => $user, 'creating' => true])
        <button type="submit" class="saas-btn-primary">{{ __('admin.create_user') }}</button>
    </form>
@endsection
